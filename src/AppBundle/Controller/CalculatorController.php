...
class CalculatorController extends Controller
{
    ...
    public function calculatorAction(Request $request)
    {
        ...
        if ($request->query->get('clear-session', false)) {
            $request->getSession()->set('calculator.processId', null);
            $request->getSession()->set('calculator.tokenId', null);

            return $this->redirectToRoute('calculator');
        }

        if ($processId = $request->getSession()->get('calculator.processId')) {
            $process = $this->getProcessStorage()->findExecution($processId);
        } else {
            $process = $this->createProcess();
        }

        if ($tokenId = $request->getSession()->get('calculator.tokenId')) {
        ...
        if ($waitTokens) {
            $request->getSession()->set('calculator.processId', $process->getId());
            $request->getSession()->set('calculator.tokenId', $waitTokens[0]->getId());
        } else {
            $request->getSession()->remove('calculator.processId');
            $request->getSession()->remove('calculator.tokenId');
        }
